...
import { patientsAPI, dashboardAPI } from '../lib/api';
import { useAuth } from '../context/AuthContext';
import { Search, ArrowRight, Activity, Plus, CheckCircle2, BellRing } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const loadData = async () => {
    setLoading(true);
    const [patientsResult, alertsResult] = await Promise.all([
      patientsAPI.list(1, 100),
      dashboardAPI.alerts(2),
    ]);
    if (patientsResult.data) {
      setPatients(patientsResult.data.items);
    }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <h1 className="text-2xl font-bold">Fisio Admin - Panel Principal</h1>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {loading ? (
                <div className="col-span-full text-center py-6 text-[var(--muted-foreground)] text-sm">Cargando...</div>
              ) : (
                [...patients]
              ...
              alerts.map((a, i) => (
                <motion.div
                  ...
                  className={`p-4 bg-[var(--card)] border rounded-2xl relative overflow-hidden flex flex-col gap-3 shadow-sm ${
                    a.remaining === 0
                      ? 'border-red-200 dark:border-red-900/50 hover:border-red-300 dark:hover:border-red-800/80'
                      : 'border-amber-200 dark:border-amber-900/50 hover:border-amber-300 dark:hover:border-amber-800/80'
                ...
                >
                  ...
                  <div className="pl-2 flex items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs font-medium text-[var(--muted-foreground)] mt-0.5 line-clamp-1" title={a.cie10Description}>{a.cie10Description}</p>
                    </div>
